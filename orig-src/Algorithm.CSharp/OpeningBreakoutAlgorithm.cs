...
package com.quantconnect.lean.Algorithm.CSharp
{
    ...
    public class OpeningBreakoutAlgorithm : QCAlgorithm
    {
        ...
        private static final boolean EnablePlotting = true;
        private static final boolean EnableOrderUpdateLogging = false;
        private static final int PricePlotFrequencyInSeconds = 15;
        
        // risk control
        private static final BigDecimal MaximumLeverage = 4;
        private static final BigDecimal GlobalStopLossPercent = 0.001m;
        private static final BigDecimal PercentProfitStartPsarTrailingStop = 0.0003m;
        private static final BigDecimal MaximumPorfolioRiskPercentPerPosition = .0025m;

        // entrance criteria
        private static final int OpeningSpanInMinutes = 3;
        private static final BigDecimal BreakoutThresholdPercent = 0.00005m;
        private static final BigDecimal AtrVolatilityThresholdPercent = 0.00275m;
        private static final BigDecimal StdVolatilityThresholdPercent = 0.005m;
        ...
        private void ScanForEntrance() {
            ...
            allowedDollarLoss = MaximumPorfolioRiskPercentPerPosition * Portfolio.TotalPortfolioValue;

            shares = (int) (allowedDollarLoss/expectedCaptureRange);

            // determine a position size based on an acceptable loss in proporton to our total portfolio value
            //shares = (int) (MaximumLeverage*MaximumPorfolioRiskPercentPerPosition*Portfolio.TotalPortfolioValue/(0.4m*ATR14));

            // max out at a little below our stated max, prevents margin calls and such
            maxShare = CalculateOrderQuantity(Symbol, MaximumLeverage);
            shares = Math.Min(shares, maxShare);

            // min out at 1x leverage
            //minShare = CalculateOrderQuantity(Symbol, MaximumLeverage/2m);
