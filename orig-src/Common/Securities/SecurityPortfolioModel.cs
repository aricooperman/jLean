    ...
    public class SecurityPortfolioModel : ISecurityPortfolioModel
    {
        ...
        */
        ...
        public virtual void ProcessFill(SecurityPortfolioManager portfolio, Security security, OrderEvent fill) {
            ...
            isLong = security.Holdings.IsLong;
            isShort = security.Holdings.IsShort;
            closedPosition = false;
            //Make local decimals to avoid any rounding errors from int multiplication
            quantityHoldings = (decimal)security.Holdings.Quantity;
            absoluteHoldingsQuantity = security.Holdings.AbsoluteQuantity;
            averageHoldingsPrice = security.Holdings.AveragePrice;
            ...
            try
            {
                ...
                feeThisOrder = Math.Abs(fill.OrderFee);
                ...
                if( closedPosition) {
                    ...
                    absoluteQuantityClosed = Math.Min(fill.AbsoluteFillQuantity, absoluteHoldingsQuantity);
                    closedSaleValueInQuoteCurrency = Math.Sign(-fill.FillQuantity)*fill.FillPrice*absoluteQuantityClosed;
                    closedCost = Math.Sign(-fill.FillQuantity)*absoluteQuantityClosed*averageHoldingsPrice;
                    conversionFactor = security.QuoteCurrency.ConversionRate*security.SymbolProperties.ContractMultiplier;
                    lastTradeProfit = (closedSaleValueInQuoteCurrency - closedCost)*conversionFactor;
