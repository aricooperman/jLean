...
namespace QuantConnect.Util
{
    ...
    public class ColorJsonConverter : TypeChangeJsonConverter<Color, string>
    {
        ...
        protected override Color Convert( String value)
        {
            ...
            if (value.Length != 7)
            {
                message = string.Format("Unable to convert '{0}' to a Color. Requires String length of 7 including the leading hashtag.", value);
                throw new FormatException(message);
            }

            red = HexToInt(value.Substring(1, 2));
            green = HexToInt(value.Substring(3, 2));
            blue = HexToInt(value.Substring(5, 2));
        ...
        private int HexToInt( String hexValue)
        {
            ...
            {
                message = string.Format("Unable to convert '{0}' to an Integer. Requires String length of 2.", hexValue);
                throw new FormatException(message);
            }
